...
\begin{solution}
	a) Youssef Cherri (ycherri) and Jonathan Kertawidjaja (jonkert)\\
...
\begin{solution}
	\begin{tabular}{ll}
		...
		Thus, $(b \rightarrow a) \land (c \rightarrow a) \equiv \neg(b \lor c) \lor a$.
	\end{tabular}
\end{solution}
...
\begin{solution}
	$s = (p \land q) \land \neg r$\\ t = $\neg q \lor r$\\ w = $\neg (p \land q) \land r$.
	\\ For s:
	\\ As it is mostly False, it can be concluded that it should be an $\land$
	...
	\\ For t:
	\\ As it is mostly True, it can be concluded that it should be an $\lor$
...
\begin{solution}
	a) Violet is the hacker. Blu and Grey's propositions contradict each other, thus one of them is lying for sure. Since the authorities know exactly one of them is lying, it has to be either Blu or Grey.
	Thus Redd and Violet are lying. Blu, therefore, did not do it. Violet is the one that did it. Regardless of whether Blu or Grey is lying.
	\\ b) Blu is the hacker. Same idea as a), but this time Redd and Violet are telling the truth. Therefore, Blu has to be the hacker.
\end{solution}
